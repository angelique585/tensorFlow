...
class CSIDataProcessor:
    ...
    def __init__(self, data_path, class_names, n_symbols=None, n_subcarriers=None, 
                 norm_type='standard', script_name='csi_resnet50_model', timestamp=None):
    ...
    def load_and_process_data(self):
        ...
        print(f"\n=== CHARGEMENT DE TOUS LES FICHIERS .MAT POUR RESNET-50 ===")
        ...
        for filename in mat_files:
            ...
            try:
                ...
                # Pour ResNet-50, nous créons des "images" 2D plus grandes
                if self.n_symbols is None:
                    # Taille minimale recommandée pour ResNet-50
                    self.n_symbols = max(32, min(224, csi_data.shape[0]))  
...
def conv_block(x, filters, kernel_size=3, stride=1, conv_shortcut=True, name=None):
    """Bloc convolutionnel avec shortcut pour ResNet-50"""
    if conv_shortcut:
...
def build_resnet50_2channel(input_shape, num_classes):
    """Construit un modèle ResNet-50 modifié pour 2 canaux (amplitude + phase)"""
    ...
    # Stage 2 - conv2_x (3 blocs)
    x = conv_block(x, 64, conv_shortcut=True, name='conv2_block1')
    x = identity_block(x, 64, name='conv2_block2')
    x = identity_block(x, 64, name='conv2_block3')

    # Stage 3 - conv3_x (4 blocs)
    x = conv_block(x, 128, stride=2, conv_shortcut=True, name='conv3_block1')
    x = identity_block(x, 128, name='conv3_block2')
    x = identity_block(x, 128, name='conv3_block3')
    x = identity_block(x, 128, name='conv3_block4')

    # Stage 4 - conv4_x (6 blocs)
    x = conv_block(x, 256, stride=2, conv_shortcut=True, name='conv4_block1')
    ...
    # Stage 5 - conv5_x (3 blocs)
    x = conv_block(x, 512, stride=2, conv_shortcut=True, name='conv5_block1')
    ...
    model = Model(inputs, outputs, name='resnet50_2channel')
    
    # Compilation avec un learning rate adapté à ResNet-50
    model.compile(
        optimizer=Adam(learning_rate=0.0001),  # Learning rate plus faible pour ResNet-50
...
def export_data_to_csv(X_combined, y, class_names, file_stats, prefix="csi_data_resnet50"):
    """Exporte les données vers des fichiers CSV"""
    print("\n=== EXPORT DES DONNÉES RESNET-50 ===")
    ...
    print(f" Données ResNet-50 2 canaux sauvées : {csv_filename}")
    ...
    if file_stats:
        ...
        files_report_filename = "files_analysis_report_resnet50.csv"
        df_files.to_csv(files_report_filename, index=False)
        print(f" Rapport des fichiers ResNet-50 sauvé : {files_report_filename}")
...
def evaluate_model_performance(model, X_test, y_test, class_names, training_time, history):
    """Évalue les performances du modèle ResNet-50 de manière complète"""
    print(f"\n=== ÉVALUATION COMPLÈTE RESNET-50 ===")
    ...
    # Taille du modèle
    model_filename = "resnet50_2channel_multiclass.keras"
    ...
    print(f"\n===  PERFORMANCES FINALES RESNET-50 (2 CANAUX) ===")
...
def main():
    """Fonction principale pour exécuter l'entraînement ResNet-50 à 2 canaux"""
    
    # === CONFIGURATION ===
    data_path = "/datos/TSFR"  # Chemin vers le dossier datos
    class_names = ['vacio', '1_persona', '2_personas', '3_personas']  # 4 classes
    n_symbols = 64  # Hauteur de l'image pour ResNet-50 (plus grande que ResNet-18)
    ...
    # === VÉRIFICATION DU FORMAT RESNET-50 ===
    print(f"\n=== PRÉPARATION POUR RESNET-50 (2 CANAUX) ===")
    ...
    csv_filename, files_report_filename = export_data_to_csv(
        X_combined, y, class_names, file_stats, "csi_data_resnet50_2channel"
    )
    ...
    # === CONSTRUCTION ET ENTRAÎNEMENT DU MODÈLE RESNET-50 ===
    print(f"\n=== CONSTRUCTION DU MODÈLE RESNET-50 (2 CANAUX) ===")
    unique_labels = np.unique(y)
    num_classes = len(unique_labels)
    
    model = build_resnet50_2channel(input_shape=X_combined.shape[1:], num_classes=num_classes)
    model.summary()

    print(f"\n=== ENTRAÎNEMENT RESNET-50 ===")
    
    # Configuration d'entraînement adaptée à ResNet-50
    ...
    performance_filename = f"resnet50_2channel_performance_report_{processor.timestamp}.csv"
    performance_df.to_csv(performance_filename, index=False)
    print(f"\n Rapport de performance sauvé : {performance_filename}")

    print(f"\n ENTRAÎNEMENT RESNET-50 (2 CANAUX) TERMINÉ AVEC SUCCÈS ")
