...
# Normalisation et reshape pour LSTM
...
# === Modèle ===
model = build_lstm_model(input_shape=(features, 1), num_classes=4)
model.summary()
...
# Inference time (1 sample)
start_inf = time.time()
_ = model.predict(np.expand_dims(X_test[0], axis=0))
inference_time_ms = (time.time() - start_inf) * 1000

# Model size
model.save("lstm_model_singlefile.keras")
model_size_mb = os.path.getsize("lstm_model_singlefile.keras") / (1024 * 1024)
...
# === Résumé complet
print("\n=== Évaluation complète du modèle LSTM ===")
